...
INPUT_CSV="${1:-}"
if [ -z "$INPUT_CSV" ]; then
  latest_dir=$(ls -1dt "$AA_DIR"/results/* 2>/dev/null | head -n1 || true)
  if [ -z "$latest_dir" ]; then
    echo "[error] no results folder found under $AA_DIR/results" >&2
    exit 1
  fi
...
OUT_CSV="$(dirname "$INPUT_CSV")/alpha_rank.csv"

awk -F',' -v out="$OUT_CSV" '
BEGIN {
  OFS=",";
  print "case,alpha_low,alpha_mid,alpha_high,n,pass,pass_rate_pct,avg_area_impr_pct,median_area_impr_pct,avg_delay_delta,worse_delay_cnt,accept_rate_pct,rej_runtime_pct_in_reject,rej_delay_pct_in_reject,rej_area_pct_in_reject" > out;
}
NR==1 { next }
{
  c=$2;
...
END {
  for (k in n) {
    ...
    printf "%s,%.2f,%.2f,%.2f,%d,%d,%.2f,%.4f,%.4f,%.4f,%d,%.2f,%.2f,%.2f,%.2f\n", k, alpha_low[k], alpha_mid[k], alpha_high[k], n[k], pass[k], pr, avg, med, add, worse[k]+0, ar, rrp, rdp, rap >> out;
  }
}
' "$INPUT_CSV"
...
{
  head -n 1 "$OUT_CSV"
  tail -n +2 "$OUT_CSV" | sort -t, -k8,8nr
} | sed -n '1,12p'
