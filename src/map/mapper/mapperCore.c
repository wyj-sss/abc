...
/**Function*************************************************************
  ...
  Synopsis    [Technology mapping for one-to-one mapping with K=2.]

  Description [One-to-one mapping with K=2 constraint.]
...
int Map_Mapping_OTO( Map_Man_t * p )
{
    ...
    // Restrict feasible cuts to K=2 for one-to-one mapping.
    if ( p->nVarsMax > 2 )
        p->nVarsMax = 2;
